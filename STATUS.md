...
stage:          done
licence:        silent
licence_where:  four places; made public because the source is dead and silent
showcase:       complete
tested_on:
workshop:
left:
...
- **`left`** — the catch-all line is replaced by three real ones, now that `TESTING.md` says
...
The three categories `left` takes: `feature` for something missing from the first pass, `defect`
...
`alive` no licence but a living source, `forbidden` a written refusal, `original` nothing carried
over.
